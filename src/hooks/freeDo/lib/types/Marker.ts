...
export interface IMarker extends BaseObject {
    ...
    add(data: IMarkerOption, fn?: Function): Promise<any>

    // 修改一个或多个标注对象
    update(data: IMarkerOption|IMarkerOption[], fn?: Function): Promise<any>
    ...
    // 设置标注的文本
    setText(id: string, newVal: string, fn?: Function): Promise<any>

    // 设置避让优先级
    setPriority(id: string, newVal: string, fn?: Function): Promise<any>

    // 设置标注的URL
    setURL(id: string, newVal: string, fn?: Function): Promise<any>

    // 设置标注的位置
    setCoordinate(id: string, newVal: Vector3, fn?: Function): Promise<any>

    // 设置是否参与遮挡剔除
    setOcclusionCull(id: string, newVal: boolean, fn?: Function): Promise<any>


    // 设置标注的位置
    setFontColor(id: string, newVal: Colors, fn?: Function): Promise<any>

    // 设置标注图片的大小
    setImageSize(id: string, newVal: Vector2, fn?: Function): Promise<any>

    // setImageSize
    setImagePath(id: string, newVal: string, fn?: Function): Promise<any>

    // 设置鼠标悬停时显示的图片路径
    setHoverImagePath(id: string, newVal: string, fn?: Function): Promise<any>

    // 隐藏标注
    hide(ids: string | string[], fn?: Function): Promise<any>

    // 隐藏所有标注
    hideAll(fn?: Function): Promise<any>

    // 显示标注
    show(ids: string | string[], fn?: Function): Promise<any>

    // 显示所有标注
    showAll(fn?: Function): Promise<any>

    // 隐藏指定标注的弹出窗口
    hidePopupWindow(ids: string | string[], fn?: Function): Promise<any>

    // 隐藏所有标注的弹出窗口
    hideAllPopupWindow(fn?: Function): Promise<any>

    // 显示指定标注的弹出窗口
    showPopupWindow(ids: string | string[], fn?: Function): Promise<any>

    // 显示所有标注的弹出窗口
    showAllPopupWindow(fn?: Function): Promise<any>

    // 设置弹窗HTML链接
    setPopupURL(id: string, newVal: string, fn?: Function): Promise<any>

    // 设置弹窗大小: [width, height]
    setPopupSize(id: string, newVal: Vector2, fn?: Function): Promise<any>

    // 设置弹窗偏移: [x, y]
    setPopupOffset(id: string, newVal: Vector2, fn?: Function): Promise<any>

    // 删除场景中所有的标注
    clear(fn?: Function): Promise<any>

    // 删除一个或多个标注对象
    delete(ids: string | string[], fn?: Function): Promise<any>


    // 自动定位到合适的观察距离
    focus(ids: string | string[], distance?: number, flyTime?: number, fn?: Function): Promise<any>

    // 自动定位到能观察所有标注对象的合适距离
    focusAll(distance?: number, flyTime?: number, fn?: Function): Promise<any>
}
